...
REPO_DIR=$DIR/".."
CIRCUIT_DIR=$DIR"/circuits/zkaa_recursive"
SETUP_DIR=$REPO_DIR"/keys/setup"
SETUP_MK=$SETUP_DIR"/setup_2^20.key"
BIG_SETUP_MK=$SETUP_DIR"/setup_2^26.key"
...
if ([ ! -f $BIG_SETUP_MK ] & $DOWNLOAD_SETUP_FROM_REMOTE); then
  ...
  axel -ac https://universal-setup.ams3.digitaloceanspaces.com/setup_2^${SETUP_POWER}.key -o $BIG_SETUP_MK || true
    ...
    $PLONKIT_BIN setup --power 26 --srs_monomial_form $BIG_SETUP_MK --overwrite
fi
popd
...
/usr/bin/time -v $PLONKIT_BIN recursive-prove -m $BIG_SETUP_MK -f $OLD_PROOF_LIST -v $CIRCUIT_DIR/vk.bin -n $CIRCUIT_DIR/recursive_proof.bin -j $CIRCUIT_DIR/recursive_proof.json --overwrite
